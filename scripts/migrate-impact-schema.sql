-- Impact Measurement Database Migration Script
-- SGE V3 GIIS - Run this to set up the complete impact measurement system
...
CREATE TABLE IF NOT EXISTS project_impact_mappings (
  id SERIAL PRIMARY KEY,
  project_id VARCHAR(255) NOT NULL,
  attribution_percentage DECIMAL(5,2) DEFAULT 0.00 CHECK (attribution_percentage >= 0 AND attribution_percentage <= 100),
  evidence_quality VARCHAR(20) DEFAULT 'medium' CHECK (evidence_quality IN ('high', 'medium', 'low')),
...
CREATE TABLE IF NOT EXISTS sdg_indicators (
  ...
  measurement_type VARCHAR(20) NOT NULL CHECK (measurement_type IN ('quantitative', 'qualitative', 'mixed')),
  created_at TIMESTAMP DEFAULT NOW(),
...
CREATE TABLE IF NOT EXISTS project_sdg_mappings (
  ...
  contribution_percentage DECIMAL(5,2) DEFAULT 0.00 CHECK (contribution_percentage >= 0 AND contribution_percentage <= 100),
  evidence TEXT[],
  created_at TIMESTAMP DEFAULT NOW(),
  updated_at TIMESTAMP DEFAULT NOW(),
  ...
  UNIQUE(project_id, sdg_goal_id, sdg_target_id, sdg_indicator_id)
);

-- Victorian Government Departments Reference Table
CREATE TABLE IF NOT EXISTS victorian_departments (
  id SERIAL PRIMARY KEY,
...
CREATE TABLE IF NOT EXISTS victorian_outcomes (
...
CREATE TABLE IF NOT EXISTS victorian_indicators (
  id VARCHAR(50) PRIMARY KEY,
...
CREATE TABLE IF NOT EXISTS project_victorian_mappings (
  id SERIAL PRIMARY KEY,
  project_id VARCHAR(255) NOT NULL,
  outcome_id VARCHAR(50) NOT NULL,
...
CREATE TABLE IF NOT EXISTS cemp_principles (
  id VARCHAR(20) PRIMARY KEY,
  code VARCHAR(20) NOT NULL,
  title VARCHAR(255) NOT NULL,
  description TEXT,
...
CREATE TABLE IF NOT EXISTS cemp_indicators (
  id VARCHAR(50) PRIMARY KEY,
...
CREATE TABLE IF NOT EXISTS project_cemp_mappings (
  id SERIAL PRIMARY KEY,
  project_id VARCHAR(255) NOT NULL,
  principle_id VARCHAR(20) NOT NULL,
  indicator_id VARCHAR(50) NOT NULL,
...
CREATE TABLE IF NOT EXISTS impact_measurements (
  ...
  framework VARCHAR(20) NOT NULL CHECK (framework IN ('SDG', 'Victorian', 'CEMP', 'Custom')),
...
CREATE TABLE IF NOT EXISTS impact_stories (
  ...
  participant_type VARCHAR(20) NOT NULL CHECK (participant_type IN ('individual', 'group', 'community')),
  impact_category VARCHAR(20) NOT NULL CHECK (impact_category IN ('social', 'economic', 'environmental', 'cultural')),
  frameworks TEXT[],
  media_files TEXT[],
  permissions JSONB,
  created_at TIMESTAMP DEFAULT NOW(),
...
CREATE TABLE IF NOT EXISTS triple_bottom_line (
  id SERIAL PRIMARY KEY,
  project_id VARCHAR(255) NOT NULL,
  measurement_date DATE NOT NULL,
  
  -- Social Impact
  employment_created INTEGER DEFAULT 0 CHECK (employment_created >= 0),
  skills_developed INTEGER DEFAULT 0 CHECK (skills_developed >= 0),
  community_engagement DECIMAL(5,2) DEFAULT 0.00 CHECK (community_engagement >= 0 AND community_engagement <= 100),
  social_cohesion DECIMAL(5,2) DEFAULT 0.00 CHECK (social_cohesion >= 0 AND social_cohesion <= 100),
  health_improvements DECIMAL(5,2) DEFAULT 0.00 CHECK (health_improvements >= 0 AND health_improvements <= 100),
  
  -- Economic Impact
  jobs_created INTEGER DEFAULT 0 CHECK (jobs_created >= 0),
  income_generated DECIMAL(12,2) DEFAULT 0.00 CHECK (income_generated >= 0),
  local_spending DECIMAL(12,2) DEFAULT 0.00 CHECK (local_spending >= 0),
  business_development DECIMAL(5,2) DEFAULT 0.00 CHECK (business_development >= 0 AND business_development <= 100),
  economic_multiplier DECIMAL(4,2) DEFAULT 1.00 CHECK (economic_multiplier >= 0),
  
  -- Environmental Impact
  carbon_reduction DECIMAL(10,2) DEFAULT 0.00 CHECK (carbon_reduction >= 0),
  waste_reduction DECIMAL(10,2) DEFAULT 0.00 CHECK (waste_reduction >= 0),
  energy_efficiency DECIMAL(5,2) DEFAULT 0.00 CHECK (energy_efficiency >= 0 AND energy_efficiency <= 100),
  biodiversity_improvement DECIMAL(5,2) DEFAULT 0.00 CHECK (biodiversity_improvement >= 0 AND biodiversity_improvement <= 100),
  sustainable_practices DECIMAL(5,2) DEFAULT 0.00 CHECK (sustainable_practices >= 0 AND sustainable_practices <= 100),
...
CREATE INDEX IF NOT EXISTS idx_project_impact_mappings_project_id ON project_impact_mappings(project_id);
CREATE INDEX IF NOT EXISTS idx_project_sdg_mappings_project_id ON project_sdg_mappings(project_id);
CREATE INDEX IF NOT EXISTS idx_project_victorian_mappings_project_id ON project_victorian_mappings(project_id);
CREATE INDEX IF NOT EXISTS idx_project_cemp_mappings_project_id ON project_cemp_mappings(project_id);
...
CREATE INDEX IF NOT EXISTS idx_impact_measurements_framework ON impact_measurements(framework);
CREATE INDEX IF NOT EXISTS idx_impact_measurements_date ON impact_measurements(measurement_date);
...
CREATE INDEX IF NOT EXISTS idx_triple_bottom_line_project_id ON triple_bottom_line(project_id);
CREATE INDEX IF NOT EXISTS idx_triple_bottom_line_date ON triple_bottom_line(measurement_date);
...
-- Insert SDG Goals (if not exists)
...
-- Insert Victorian Departments (if not exists)
...
-- Insert CEMP Principles (if not exists)
...
DROP TRIGGER IF EXISTS update_project_impact_mappings_updated_at ON project_impact_mappings;
CREATE TRIGGER update_project_impact_mappings_updated_at 
    BEFORE UPDATE ON project_impact_mappings 
    FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();

DROP TRIGGER IF EXISTS update_project_sdg_mappings_updated_at ON project_sdg_mappings;
CREATE TRIGGER update_project_sdg_mappings_updated_at 
    BEFORE UPDATE ON project_sdg_mappings 
    FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();

DROP TRIGGER IF EXISTS update_project_victorian_mappings_updated_at ON project_victorian_mappings;
CREATE TRIGGER update_project_victorian_mappings_updated_at 
    BEFORE UPDATE ON project_victorian_mappings 
    FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();

DROP TRIGGER IF EXISTS update_project_cemp_mappings_updated_at ON project_cemp_mappings;
CREATE TRIGGER update_project_cemp_mappings_updated_at 
    BEFORE UPDATE ON project_cemp_mappings 
    FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();

DROP TRIGGER IF EXISTS update_impact_measurements_updated_at ON impact_measurements;
CREATE TRIGGER update_impact_measurements_updated_at 
    BEFORE UPDATE ON impact_measurements 
    FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();

DROP TRIGGER IF EXISTS update_impact_stories_updated_at ON impact_stories;
CREATE TRIGGER update_impact_stories_updated_at 
    BEFORE UPDATE ON impact_stories 
    FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();

DROP TRIGGER IF EXISTS update_triple_bottom_line_updated_at ON triple_bottom_line;
...
WHERE schemaname = 'public' 
  AND tablename LIKE '%impact%' 
  OR tablename LIKE '%sdg%' 
  OR tablename LIKE '%victorian%' 
  OR tablename LIKE '%cemp%'
ORDER BY tablename;

-- Verify sample data was inserted
SELECT 'SDG Goals' as table_name, COUNT(*) as count FROM sdg_goals
UNION ALL
SELECT 'Victorian Departments' as table_name, COUNT(*) as count FROM victorian_departments
UNION ALL
SELECT 'CEMP Principles' as table_name, COUNT(*) as count FROM cemp_principles;
...
DO $$
BEGIN
  RAISE NOTICE 'Impact Measurement Database Migration Completed Successfully!';
  RAISE NOTICE 'Tables created: project_impact_mappings, sdg_goals, victorian_departments, cemp_principles, etc.';
  RAISE NOTICE 'Indexes created for performance optimization';
  RAISE NOTICE 'Sample data inserted for SDG, Victorian Government, and CEMP frameworks';
  RAISE NOTICE 'Views and triggers created for data integrity';
  RAISE NOTICE 'Your impact measurement system is ready to use!';
END $$;
