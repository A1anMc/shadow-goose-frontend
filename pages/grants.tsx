...
import { grantService, Grant, GrantApplication, GrantRecommendation, GrantSearchFilters } from '../src/lib/grants';

export default function Grants() {
  const router = useRouter();
  const branding = getBranding();
  const [activeTab, setActiveTab] = useState<'finder' | 'applications' | 'recommendations'>('finder');
  ...
  const loadData = async () => {
    try {
      setLoading(true);
      const [grantsData, applicationsData, recommendationsData, categoriesData] = await Promise.all([
        ...
        {activeTab === 'finder' && (
          <div>
            ...
            <div className="bg-white rounded-lg shadow p-6 mb-8">
              <h2 className="text-lg font-semibold text-gray-900 mb-4">Search Grants</h2>
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                ...
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) => setSearchFilters({ ...searchFilters, min_amount: Number(e.target.value) })}
                    className="w-full border border-gray-300 rounded-md px-3 py-2 focus:outline-none focus:ring-2 focus:ring-sg-primary"
                  />
                </div>
                <div>
                ...
                <div key={grant.id} className="bg-white rounded-lg shadow p-6">
                  <div className="flex justify-between items-start mb-4">
                    ...
                    <div className="text-right">
                      <div className="text-2xl font-bold text-sg-primary">{formatCurrency(grant.amount)}</div>
                      <span className={`inline-block px-2 py-1 rounded-full text-xs font-medium ${getStatusColor(grant.status)}`}>
                        {grant.status}
                      </span>
                    </div>
                  </div>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-4">
                    ...
                    <div>
                      ...
                      <p className="text-sm text-gray-900">{grant.success_score}%</p>
                    </div>
                  </div>
          ...
          <div>
            <div className="bg-white rounded-lg shadow p-6 mb-8">
              ...
              {applications.length === 0 ? (
                <p className="text-gray-500">No applications yet. Start by finding grants in the Grant Finder tab.</p>
              ) : (
                    ...
                    return (
                      <div key={application.id} className="border border-gray-200 rounded-lg p-4">
                        <div className="flex justify-between items-start">
                          <div>
                            <h3 className="font-semibold text-gray-900">{grant?.name || 'Unknown Grant'}</h3>
                            <p className="text-sm text-gray-600 mt-1">
                              Applied on {formatDate(application.created_at)}
                            </p>
                          </div>
                          <div className="text-right">
                            <span className={`inline-block px-2 py-1 rounded-full text-xs font-medium ${getApplicationStatusColor(application.status)}`}>
          ...
          <div>
            <div className="bg-white rounded-lg shadow p-6 mb-8">
              ...
              <p className="text-gray-600 mb-6">
                Our AI analyzes your profile and finds the best grants for your organization.
              </p>

              {recommendations.length === 0 ? (
                <p className="text-gray-500">No recommendations available. Try updating your profile or searching for grants.</p>
              ) : (
                <div className="space-y-6">
                  {recommendations.map((rec, index) => (
                    <div key={index} className="border border-gray-200 rounded-lg p-6">
                      <div className="flex justify-between items-start mb-4">
                        <div>
                          <h3 className="text-xl font-semibold text-gray-900">{rec.grant.name}</h3>
                          <p className="text-gray-600 mt-1">{rec.grant.description}</p>
                        </div>
                        <div className="text-right">
                          <div className="text-2xl font-bold text-sg-primary">{formatCurrency(rec.grant.amount)}</div>
                          <div className="text-sm text-gray-500">Match Score: {rec.match_score}%</div>
                        </div>
                      </div>

                      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                        <div>
                          <span className="text-sm font-medium text-gray-500">Why This Grant?</span>
                        ...
                        <div>
                          ...
                          <div className="mt-1">
                            ...
                            <p className="text-sm text-gray-900 mt-1">{rec.success_probability}% chance of success</p>
                          </div>
                        </div>
                      </div>
