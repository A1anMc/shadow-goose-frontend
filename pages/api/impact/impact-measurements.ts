...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  if (req.method === 'GET') {
    try {
      ...
      console.error('Error fetching impact measurements:', error);
    ...
    try {
      ...
      console.error('Error updating impact measurement:', error);
      res.status(500).json({ error: 'Internal server error' });
    }
    ...
    try {
      ...
      console.error('Error deleting impact measurement:', error);
      res.status(500).json({ error: 'Internal server error' });
    }
  } else {
